...
- **Shopping Assistant vs Cart Item workflow** — Same storefront chat in **`ecommerce`**. **Shopping Assistant** (`shoppingAgent` in **`ai-server`**) retrieves **Products** and may start a **Cart Item workflow** as a tool. The workflow emits A2UI from step results; **`ecommerce`** `CartStore` writes the **Cart Item**. Not a sub-agent; not **Checkout**. Decision records: [0006](./docs/ecommerce/adr/0006-cart-item-workflow-stock-and-cartstore.md), [0007](./docs/ecommerce/adr/0007-cart-item-workflow-mastra-not-subagent.md).
