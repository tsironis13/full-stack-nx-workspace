 ...
 *
 * - **Write side** — re-exported `cartCatalogEvents` event group: foreign code
 *   dispatches these events; Cart's `GuestCartStore` reduces them. Callers must
 *   never import `GuestCartStore` or call its methods directly.
 *
 * - **Read side** — `CartAclReadAdapter`: an injectable service exposing narrow,
 *   stable computed signals (badge count, per-item quantities) without leaking
 *   the full `GuestCartStore` type to consumers.
 *
...
export { cartCatalogEvents } from './public-api';
